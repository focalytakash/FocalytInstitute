...
router.route("/institute").get(auth1, async (req, res) => {
  let view = false
  if(req.session.user.role === 10){
    view = true
  }
  ...
  return res.render(`${req.vPath}/admin/miPieCoins/institute/institute`, {
    menu: "institute",
    offers,
    isChecked,
    totalPages,
...
router.route("/institute/addEditOffer/:offerId?").get(auth1, async (req, res) => {
  let { offerId } = req.params;
  let offer = {};
  if (offerId) {
    offer = await CoinsOffers.findById(offerId);
  }
  return res.render(`${req.vPath}/admin/miPieCoins/institute/addEditOffer`, {
    menu: "instituteCoins",
    offer,
  });
});

router.route("/institute/viewOffer/:offerId").get(auth1, async (req, res) => {
  try {
    let { offerId } = req.params;
    let offer = await CoinsOffers.findById(offerId);
    if(!offer){
      req.flash("error", 'Offer not found');
      return res.status(302).redirect("/admin/Coins/institute");
    }
    return res.render(`${req.vPath}/admin/miPieCoins/institute/viewOffer`, {
      menu: "instituteCoins",
      offer,
    });
  } catch (err) {
...
router.get('/availedInstitutes/:id',auth1 , async(req,res)=>{
  try{
    const _offer = req.params.id;
    let data = req.query
    ...
    data.name?.split('').some(char => 
      ...
      if (name && numberCheck) {
        filter["$or"] = [
          { "_college.name":{ "$regex": name, "$options": "i"}},
        ]
      }
      if (name && !numberCheck ) {
        filter["$or"] = [
    ...
    sort[`${value}`] = Number(order)
      ...
      let agg = [
        ...
        },
        {
          ...
          {
            from : 'users',
            localField: '_college._concernPerson',
            foreignField: '_id',
            as: 'user'
          }
        },
    ...
    let payments = PaymentData[0].data
    const totalPages = Math.ceil(count / perPage);
     return res.render(`${req.vPath}/admin/miPieCoins/institute/instituteList`, {
      menu: "instituteCoins",
      payments,
      data,
      totalPages,
